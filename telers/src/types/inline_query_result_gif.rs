...
impl InlineQueryResultGif {
    ...
    /// A valid URL for the GIF file
    #[must_use]
    pub fn gif_url<T: Into<Box<str>>>(self, val: T) -> Self {
        let mut this = self;
        this.gif_url = val.into();
        this
    }

    /// Width of the GIF
    #[must_use]
    pub fn gif_width<T: Into<i64>>(self, val: T) -> Self {
        let mut this = self;
        this.gif_width = Some(val.into());
        this
    }

    /// Width of the GIF
    #[must_use]
    pub fn gif_width_option<T: Into<i64>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.gif_width = val.map(Into::into);
        this
    }

    /// Height of the GIF
    #[must_use]
    pub fn gif_height<T: Into<i64>>(self, val: T) -> Self {
        let mut this = self;
        this.gif_height = Some(val.into());
        this
    }

    /// Height of the GIF
    #[must_use]
    pub fn gif_height_option<T: Into<i64>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.gif_height = val.map(Into::into);
        this
    }

    /// Duration of the GIF in seconds
    #[must_use]
    pub fn gif_duration<T: Into<i64>>(self, val: T) -> Self {
        let mut this = self;
        this.gif_duration = Some(val.into());
        this
    }

    /// Duration of the GIF in seconds
    #[must_use]
    pub fn gif_duration_option<T: Into<i64>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.gif_duration = val.map(Into::into);
        this
    }
