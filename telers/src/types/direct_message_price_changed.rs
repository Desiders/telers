...
impl DirectMessagePriceChanged {
    ...
    /// `true`, if direct messages are enabled for the channel chat; false otherwise
    #[must_use]
    pub fn are_direct_messages_enabled<T: Into<bool>>(self, val: T) -> Self {
        let mut this = self;
        this.are_direct_messages_enabled = val.into();
        this
    }

    /// The new number of Telegram Stars that must be paid by users for each direct message sent to the channel. Does not apply to users who have been exempted by administrators. Defaults to 0.
    #[must_use]
    pub fn direct_message_star_count<T: Into<i64>>(self, val: T) -> Self {
        let mut this = self;
        this.direct_message_star_count = Some(val.into());
        this
    }

    /// The new number of Telegram Stars that must be paid by users for each direct message sent to the channel. Does not apply to users who have been exempted by administrators. Defaults to 0.
    #[must_use]
    pub fn direct_message_star_count_option<T: Into<i64>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.direct_message_star_count = val.map(Into::into);
        this
    }
}
