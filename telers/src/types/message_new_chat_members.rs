...
/// # Notes
/// This object represents a service message from original message field `new_chat_members`.
/// # Documentation
...
pub struct MessageNewChatMembers {
    ...
    /// New members that were added to the group or supergroup and information about them (the bot itself may be one of these members)
    pub new_chat_members: Box<[crate::types::User]>,
}
impl MessageNewChatMembers {
    /// Creates a new `MessageNewChatMembers`.
    ///
    /// # Arguments
    ...
    >(
        ...
        Self {
            ...
            reply_markup: None,
            new_chat_members: new_chat_members.into_iter().map(Into::into).collect(),
        }
    }
    ...
    /// Adds multiple elements.
    #[must_use]
    pub fn new_chat_members<T: Into<Box<[crate::types::User]>>>(self, val: T) -> Self {
        let mut this = self;
        this.new_chat_members = this
            .new_chat_members
            .into_vec()
            .into_iter()
            .chain(val.into())
    ...
    /// Adds a single element.
    #[must_use]
    pub fn new_chat_member<T: Into<crate::types::User>>(self, val: T) -> Self {
        let mut this = self;
        this.new_chat_members = this
            .new_chat_members
            .into_vec()
