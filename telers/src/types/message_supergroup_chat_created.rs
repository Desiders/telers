...
/// # Notes
/// This object represents a service message from original message field `supergroup_chat_created`.
/// # Documentation
...
impl MessageSupergroupChatCreated {
    /// Creates a new `MessageSupergroupChatCreated`.
    ///
    /// # Arguments
    ...
    #[must_use]
    pub fn new<T0: Into<i64>, T1: Into<i64>, T2: Into<crate::types::Chat>, T3: Into<bool>>(
        message_id: T0,
        date: T1,
        chat: T2,
        supergroup_chat_created: T3,
    ) -> Self {
        Self {
            ...
            reply_markup: None,
            supergroup_chat_created: supergroup_chat_created.into(),
        }
    }
    ...
    /// Service message: the supergroup has been created. This field can't be received in a message coming through updates, because bot can't be a member of a supergroup when it is created. It can only be found in `reply_to_message` if someone replies to a very first message in a directly created supergroup.
    #[must_use]
    pub fn supergroup_chat_created<T: Into<bool>>(mut self, val: T) -> Self {
        self.supergroup_chat_created = val.into();
        self
    }
}
