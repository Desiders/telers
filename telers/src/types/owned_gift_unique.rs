...
impl OwnedGiftUnique {
    ...
    /// `true`, if the gift can be transferred to another owner; for gifts received on behalf of business accounts only
    #[must_use]
    pub fn can_be_transferred<T: Into<bool>>(self, val: T) -> Self {
        let mut this = self;
        this.can_be_transferred = Some(val.into());
        this
    }

    /// `true`, if the gift can be transferred to another owner; for gifts received on behalf of business accounts only
    #[must_use]
    pub fn can_be_transferred_option<T: Into<bool>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.can_be_transferred = val.map(Into::into);
        this
    }
