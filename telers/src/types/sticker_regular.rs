...
impl StickerRegular {
    ...
    /// For premium regular stickers, premium animation for the sticker
    #[must_use]
    pub fn premium_animation<T: Into<crate::types::File>>(mut self, val: T) -> Self {
        self.premium_animation = Some(val.into());
        self
    }

    /// For premium regular stickers, premium animation for the sticker
    #[must_use]
    pub fn premium_animation_option<T: Into<crate::types::File>>(mut self, val: Option<T>) -> Self {
        self.premium_animation = val.map(Into::into);
        self
    }
