...
/// # Notes
/// This object represents a service message from original message field `channel_chat_created`.
/// # Documentation
...
impl MessageChannelChatCreated {
    /// Creates a new `MessageChannelChatCreated`.
    ///
    /// # Arguments
    ...
    pub fn new<T0: Into<i64>, T1: Into<i64>, T2: Into<crate::types::Chat>, T3: Into<bool>>(
        ...
        Self {
            ...
            reply_markup: None,
            channel_chat_created: channel_chat_created.into(),
        }
    }
    ...
    /// Service message: the channel has been created. This field can't be received in a message coming through updates, because bot can't be a member of a channel when it is created. It can only be found in `reply_to_message` if someone replies to a very first message in a channel.
    #[must_use]
    pub fn channel_chat_created<T: Into<bool>>(mut self, val: T) -> Self {
        self.channel_chat_created = val.into();
        self
    }
}
