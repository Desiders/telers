...
impl LoginUrl {
    ...
    /// New text of the button in forwarded messages
    #[must_use]
    pub fn forward_text<T: Into<Box<str>>>(mut self, val: T) -> Self {
    ...
    /// New text of the button in forwarded messages
    #[must_use]
    pub fn forward_text_option<T: Into<Box<str>>>(mut self, val: Option<T>) -> Self {
        self.forward_text = val.map(Into::into);
        self
    }
    ...
    /// Pass `true` to request the permission for your bot to send messages to the user
    #[must_use]
    pub fn request_write_access<T: Into<bool>>(mut self, val: T) -> Self {
        self.request_write_access = Some(val.into());
        self
    }

    /// Pass `true` to request the permission for your bot to send messages to the user
    #[must_use]
    pub fn request_write_access_option<T: Into<bool>>(mut self, val: Option<T>) -> Self {
        self.request_write_access = val.map(Into::into);
        self
    }
}
