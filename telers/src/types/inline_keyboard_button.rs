...
impl InlineKeyboardButton {
    ...
    /// Data to be sent in a callback query to the bot when the button is pressed, 1-64 bytes
    #[must_use]
    pub fn callback_data<T: Into<Box<str>>>(self, val: T) -> Self {
        let mut this = self;
        this.callback_data = Some(val.into());
        this
    }

    /// Data to be sent in a callback query to the bot when the button is pressed, 1-64 bytes
    #[must_use]
    pub fn callback_data_option<T: Into<Box<str>>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.callback_data = val.map(Into::into);
        this
    }
    ...
    /// An HTTPS URL used to automatically authorize the user. Can be used as a replacement for the Telegram Login Widget.
    #[must_use]
    pub fn login_url<T: Into<crate::types::LoginUrl>>(self, val: T) -> Self {
        let mut this = self;
        this.login_url = Some(val.into());
        this
    }

    /// An HTTPS URL used to automatically authorize the user. Can be used as a replacement for the Telegram Login Widget.
    #[must_use]
    pub fn login_url_option<T: Into<crate::types::LoginUrl>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.login_url = val.map(Into::into);
        this
    }

    /// If set, pressing the button will prompt the user to select one of their chats, open that chat and insert the bot's username and the specified inline query in the input field. May be empty, in which case just the bot's username will be inserted. Not supported for messages sent in channel direct messages chats and on behalf of a business account.
    #[must_use]
    pub fn switch_inline_query<T: Into<Box<str>>>(self, val: T) -> Self {
        let mut this = self;
        this.switch_inline_query = Some(val.into());
        this
    }

    /// If set, pressing the button will prompt the user to select one of their chats, open that chat and insert the bot's username and the specified inline query in the input field. May be empty, in which case just the bot's username will be inserted. Not supported for messages sent in channel direct messages chats and on behalf of a business account.
    #[must_use]
    pub fn switch_inline_query_option<T: Into<Box<str>>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.switch_inline_query = val.map(Into::into);
        this
    }

    /// If set, pressing the button will insert the bot's username and the specified inline query in the current chat's input field. May be empty, in which case only the bot's username will be inserted. This offers a quick way for the user to open your bot in inline mode in the same chat - good for selecting something from multiple options. Not supported in channels and for messages sent in channel direct messages chats and on behalf of a business account.
    #[must_use]
    pub fn switch_inline_query_current_chat<T: Into<Box<str>>>(self, val: T) -> Self {
        let mut this = self;
        this.switch_inline_query_current_chat = Some(val.into());
        this
    }
    ...
    pub fn switch_inline_query_current_chat_option<T: Into<Box<str>>>(
        ...
        let mut this = self;
        this.switch_inline_query_current_chat = val.map(Into::into);
        this
    }
    ...
    pub fn switch_inline_query_chosen_chat<T: Into<crate::types::SwitchInlineQueryChosenChat>>(
        ...
        let mut this = self;
        this.switch_inline_query_chosen_chat = Some(val.into());
        this
    }
    ...
    >(
        ...
        let mut this = self;
        this.switch_inline_query_chosen_chat = val.map(Into::into);
        this
    }

    /// Description of the button that copies the specified text to the clipboard.
    #[must_use]
    pub fn copy_text<T: Into<crate::types::CopyTextButton>>(self, val: T) -> Self {
        let mut this = self;
        this.copy_text = Some(val.into());
        this
    }

    /// Description of the button that copies the specified text to the clipboard.
    #[must_use]
    pub fn copy_text_option<T: Into<crate::types::CopyTextButton>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.copy_text = val.map(Into::into);
        this
    }

    /// Description of the game that will be launched when the user presses the button. NOTE: This type of button must always be the first button in the first row.
    #[must_use]
    pub fn callback_game<T: Into<crate::types::CallbackGame>>(self, val: T) -> Self {
        let mut this = self;
        this.callback_game = Some(val.into());
        this
    }

    /// Description of the game that will be launched when the user presses the button. NOTE: This type of button must always be the first button in the first row.
    #[must_use]
    pub fn callback_game_option<T: Into<crate::types::CallbackGame>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.callback_game = val.map(Into::into);
        this
    }

    /// Specify `true`, to send a Pay button. Substrings `⭐` and `XTR` in the buttons's text will be replaced with a Telegram Star icon. NOTE: This type of button must always be the first button in the first row and can only be used in invoice messages.
    #[must_use]
    pub fn pay<T: Into<bool>>(self, val: T) -> Self {
        let mut this = self;
        this.pay = Some(val.into());
        this
    }

    /// Specify `true`, to send a Pay button. Substrings `⭐` and `XTR` in the buttons's text will be replaced with a Telegram Star icon. NOTE: This type of button must always be the first button in the first row and can only be used in invoice messages.
    #[must_use]
    pub fn pay_option<T: Into<bool>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.pay = val.map(Into::into);
        this
    }
}
