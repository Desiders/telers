...
/// Upon receiving a message with this object, Telegram clients will remove the current custom keyboard and display the default letter-keyboard. By default, custom keyboards are displayed until a new keyboard is sent by a bot. An exception is made for one-time keyboards that are hidden immediately after the user presses a button (see [`crate::types::ReplyKeyboardMarkup`]). Not supported in channels and for messages sent on behalf of a Telegram Business account.
/// # Documentation
...
impl ReplyKeyboardRemove {
    ...
    /// Requests clients to remove the custom keyboard (user will not be able to summon this keyboard; if you want to hide the keyboard from sight but keep it accessible, use `one_time_keyboard` in [`crate::types::ReplyKeyboardMarkup`])
    #[must_use]
    pub fn remove_keyboard<T: Into<bool>>(self, val: T) -> Self {
        let mut this = self;
        this.remove_keyboard = val.into();
        this
    }
