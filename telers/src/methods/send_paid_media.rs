...
impl SendPaidMedia {
    ...
    pub fn new<
        ...
        T1: Into<u16>,
        T2Item: Into<crate::types::InputPaidMedia>,
    ...
    #[must_use]
    pub fn media<TItem: Into<crate::types::InputPaidMedia>, T: IntoIterator<Item = TItem>>(
        self,
        val: T,
    ) -> Self {
        let mut this = self;
    ...
    pub fn payload<T: Into<Box<str>>>(self, val: T) -> Self {
        let mut this = self;
        this.payload = Some(val.into());
        this
    }

    /// Bot-defined paid media payload, 0-128 bytes. This will not be displayed to the user, use it for your internal processes.
    #[must_use]
    pub fn payload_option<T: Into<Box<str>>>(self, val: Option<T>) -> Self {
        let mut this = self;
        this.payload = val.map(Into::into);
        this
    }
...
impl super::TelegramMethod for SendPaidMedia {
    type Method = Self;
    type Return = crate::types::Message;

    fn build_request<Client>(mut self, _bot: &Bot<Client>) -> super::Request<Self::Method> {
        let mut files = vec![];
        super::prepare_input_paid_media_group(&mut files, self.media.iter_mut().collect());
        super::Request::new("sendPaidMedia", self, Some(files))
    }
}
