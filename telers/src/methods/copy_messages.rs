...
impl CopyMessages {
    ...
    /// Pass `true` to copy the messages without their captions
    #[must_use]
    pub fn remove_caption<T: Into<bool>>(mut self, val: T) -> Self {
        self.remove_caption = Some(val.into());
        self
    }

    /// Pass `true` to copy the messages without their captions
    #[must_use]
    pub fn remove_caption_option<T: Into<bool>>(mut self, val: Option<T>) -> Self {
        self.remove_caption = val.map(Into::into);
        self
    }
}
