...
impl SendVideoNote {
    #[must_use]
    pub fn new(chat_id: impl Into<ChatIdKind>, video_note: impl Into<InputFile>) -> Self {
        Self {
    ...
    #[must_use]
    pub fn video_note(self, val: impl Into<InputFile>) -> Self {
        Self {
            video_note: val.into(),
            ..self
        }
...
impl TelegramMethod for SendVideoNote {
    ...
    fn build_request<Client>(mut self, _bot: &Bot<Client>) -> Request<Self::Method> {
        let mut files = vec![];
        prepare_file(&mut files, &mut self.video_note);
...
impl AsRef<SendVideoNote> for SendVideoNote {
    fn as_ref(&self) -> &Self {
        self
    }
}
