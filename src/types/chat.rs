...
pub struct Chat {
    ...
    /// *Optional*. Primary invite link, for groups, supergroups and channel chats. Returned only in `aiogram_rs.methods.get_chat.GetChat`.
    pub invite_link: Option<String>,
    /// *Optional*. The most recent pinned message (by sending date). Returned only in `aiogram_rs.methods.get_chat.GetChat`.
    pub pinned_message: Option<Message>,
    /// *Optional*. Default chat member permissions, for groups and supergroups. Returned only in `aiogram_rs.methods.get_chat.GetChat`.
    pub permissions: Option<ChatPermissions>,
    /// *Optional*. For supergroups, the minimum allowed delay between consecutive messages sent by each unpriviledged user; in seconds. Returned only in `aiogram_rs.methods.get_chat.GetChat`.
    ...
    /// *Optional*. For supergroups, the location to which the supergroup is connected. Returned only in `aiogram_rs.methods.get_chat.GetChat`.
