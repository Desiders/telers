...
- `photo_handler` (a `Handler` on the message observer taking `MessagePhoto`) picks the last, largest `PhotoSize`, passes it to `bot.download(...)` and reads the whole file into memory with `.bytes()`. Any object with a `file_id` implements `FileIdGetter`, so the `PhotoSize` is passed directly.
- `document_handler` (taking `MessageDocument`) derives the destination name from `Document::file_name`, falling back to `file_unique_id`, and streams the download straight to disk with `.to_path(...)`.
...
- `fallback_handler` answers everything else. The `Dispatcher` is built with `allowed_update(UpdateType::Message)` and run via `run_polling`.
